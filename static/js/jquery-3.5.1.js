    ...
    for (; i < length; i++) {
      ...
      if ((options = arguments[i]) != null) {
        ...
        for (name in options) {
          ...
          if (
            deep && copy && (jQuery.isPlainObject(copy) ||
      ...
      if (!rneedsContext.test(selectors)) {
        for (; i < l; i++) {
          for (cur = this[i]; cur && cur !== context; cur = cur.parentNode) {
            ...
            if (
              cur.nodeType < 11 && (targets ? targets.index(cur) > -1 : // Don't pass non-elements to Sizzle
                cur.nodeType === 1 &&
 ...
 */
    ...
    dispatch: function (nativeEvent) {
      ...
      while ((matched = handlerQueue[i++]) && !event.isPropagationStopped()) {
        ...
        while (
          ...
          if (
            !event.rnamespace || handleObj.namespace === false ||
  ...
  jQuery.each({
    ...
    which: function (event) {
      ...
      if (
        !event.which && button !== undefined && rmouseEvent.test(event.type)
      ) {
        if (button & 1) {
          return 1;
        }
    ...
    html: function (value) {
      return access(
        ...
        function (value) {
          ...
          if (
            typeof value === "string" && !rnoInnerhtml.test(value) &&
  ...
  jQuery.each(["height", "width"], function (_i, dimension) {
    jQuery.cssHooks[dimension] = {
      ...
      set: function (elem, value, extra) {
        ...
        if (
          subtract && (matches = rcssNum.exec(value)) &&
          (matches[3] || "px") !== "px"
        ) {
  ...
  jQuery.each({
    ...
    jQuery.cssHooks[prefix + suffix] = {
      expand: function (value) {
        ...
        for (; i < 4; i++) {
          expanded[prefix + cssExpand[i] + suffix] = parts[i] || parts[i - 2] ||
            parts[0];
        }
    ...
    for (; index < length; index++) {
      ...
      if (result) {
        if (isFunction(result.stop)) {
          jQuery._queueHooks(animation.elem, animation.opts.queue).stop = result
            .stop.bind(result);
        }
        return result;
      }
      ...
      return {
        send: function (headers, complete) {
          ...
          callback = function (type) {
            return function () {
              if (callback) {
                callback = errorCallback = xhr.onload = xhr.onerror = xhr
                  .onabort = xhr.ontimeout = xhr.onreadystatechange = null;
  ...
  jQuery.fn.extend({
    ...
    position: function () {
      ...
      if (jQuery.css(elem, "position") === "fixed") {
        ...
        if (
          offsetParent && offsetParent !== elem && offsetParent.nodeType === 1
        ) {
    ...
    offsetParent: function () {
      return this.map(function () {
        ...
        while (
          offsetParent && jQuery.css(offsetParent, "position") === "static"
        ) {
